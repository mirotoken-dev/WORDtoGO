...
// ─── Floating orb config ──────────────────────────────────────────────────────
const ORB_CONFIG = [
  { id: 0, size: 180, x: 5, y: 8, color: "oklch(0.82 0.17 84 / 0.07)", dur: 9, dx: 18, dy: 22 },
  { id: 1, size: 100, x: 80, y: 15, color: "oklch(0.90 0.18 84 / 0.09)", dur: 7, dx: -14, dy: 18 },
  { id: 2, size: 140, x: 60, y: 70, color: "oklch(0.82 0.17 84 / 0.06)", dur: 11, dx: 20, dy: -16 },
  { id: 3, size: 70, x: 25, y: 55, color: "oklch(0.92 0.18 84 / 0.10)", dur: 6, dx: -12, dy: 20 },
  { id: 4, size: 50, x: 90, y: 45, color: "oklch(0.88 0.16 84 / 0.12)", dur: 5, dx: -18, dy: -14 },
  { id: 5, size: 90, x: 45, y: 90, color: "oklch(0.82 0.17 84 / 0.08)", dur: 8, dx: 16, dy: -20 },
  { id: 6, size: 120, x: 15, y: 35, color: "oklch(0.68 0.22 290 / 0.07)", dur: 10, dx: 14, dy: 18 },
  { id: 7, size: 80, x: 70, y: 5, color: "oklch(0.72 0.22 320 / 0.08)", dur: 7, dx: -10, dy: 22 },
  { id: 8, size: 60, x: 50, y: 40, color: "oklch(0.68 0.24 264 / 0.06)", dur: 9, dx: 20, dy: -12 },
  { id: 9, size: 110, x: 85, y: 80, color: "oklch(0.65 0.22 280 / 0.07)", dur: 12, dx: -16, dy: -18 },
  { id: 10, size: 28, x: 30, y: 20, color: "oklch(0.97 0 0 / 0.08)", dur: 4, dx: 10, dy: 14 },
  { id: 11, size: 20, x: 75, y: 60, color: "oklch(0.97 0 0 / 0.07)", dur: 5, dx: -8, dy: 12 },
  { id: 12, size: 16, x: 55, y: 85, color: "oklch(0.95 0.18 84 / 0.10)", dur: 4, dx: 14, dy: -10 },
  { id: 13, size: 24, x: 10, y: 70, color: "oklch(0.97 0 0 / 0.06)", dur: 6, dx: 18, dy: -8 },
  { id: 14, size: 18, x: 92, y: 25, color: "oklch(0.95 0.18 84 / 0.09)", dur: 5, dx: -12, dy: 16 },
] as const;
    ...
    <div aria-hidden="true" className="pointer-events-none fixed inset-0 overflow-hidden" style={{ zIndex: 0 }}>
      {ORB_CONFIG.map((orb) => (
        <motion.div
          ...
          transition={{ duration: orb.dur, repeat: Number.POSITIVE_INFINITY, ease: "easeInOut", delay: orb.id * 0.4 }}
        />
      ))}
    </div>
  );
    ...
    <div className="min-h-screen flex flex-col bg-background relative">
      ...
      >
        ...
        >
          ...
          <p className="text-muted-foreground font-body text-lg mt-2">Select who's learning today!</p>
        </motion.div>
      </div>

      {/* Profiles */}
      <div className="flex-1 px-5 py-6 flex flex-col gap-4 relative" style={{ zIndex: 1 }}>
        {profiles.length === 0 && !creating && (
          <motion.div data-ocid="profiles.empty_state" className="text-center py-12" initial={{ opacity: 0 }} animate={{ opacity: 1 }}>
            <div className="text-6xl mb-4">👶</div>
            <p className="text-xl font-display font-bold text-foreground">No profiles yet!</p>
            <p className="text-sm font-body text-muted-foreground mt-1">Create a profile to start learning.</p>
          </motion.div>
        )}
        ...
        {profiles.map((profile, idx) => (
          <motion.div
            ...
            style={{ boxShadow: "0 4px 16px oklch(0 0 0 / 0.4), 0 0 0 1px oklch(0.82 0.17 84 / 0.08) inset" }}
          ...
          >
            ...
            >
              ...
              <div className="min-w-0">
                <p className="text-xl font-display font-bold text-foreground truncate">{profile.name}</p>
        ...
        <AnimatePresence>
          {profiles.length >= 3 && !creating ? (
            <motion.div
              ...
              animate={{ opacity: 1 }}
            >
              <p className="text-base font-display font-bold text-muted-foreground">Maximum 3 learners reached</p>
              <p className="text-sm font-body text-muted-foreground mt-1">Delete a profile to add a new one.</p>
            </motion.div>
          ) : creating ? (
            <motion.div
              data-ocid="profiles.create_form"
              className="bg-card rounded-3xl p-5 border border-[oklch(0.82_0.17_84/0.2)]"
              style={{ boxShadow: "0 8px 32px oklch(0 0 0 / 0.5), 0 0 0 1px oklch(0.82 0.17 84 / 0.1) inset" }}
              initial={{ scale: 0.95, opacity: 0 }}
              animate={{ scale: 1, opacity: 1 }}
              exit={{ scale: 0.95, opacity: 0 }}
            >
              <h2 className="text-xl font-display font-bold text-foreground mb-4">New Learner</h2>
              <div className="mb-4">
                <label htmlFor="profile-name" className="block text-sm font-display font-semibold text-foreground mb-2">Name</label>
                <input
                  id="profile-name"
                  ref={inputRef}
              ...
              <div className="mb-5">
                <p className="block text-sm font-display font-semibold text-foreground mb-2">Avatar</p>
            ...
            <motion.button
              ...
              onClick={() => { playTapSound(); setCreating(true); setTimeout(() => inputRef.current?.focus(), 50); }}
      ...
      <footer className="py-3 text-center bg-card/60 border-t border-[oklch(0.82_0.17_84/0.15)] relative" style={{ zIndex: 1 }}>
